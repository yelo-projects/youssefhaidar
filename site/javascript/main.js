...
jQuery(function($){
		...
		var $projectsContainer = $('#ItemsList.projects, #ItemsList.articles'), $projects,iLnH, $sorters;
		var groupsHeaders, filter;
		var $itemsType = $projectsContainer.hasClass('projects') ? 'projects' : 'articles';
		...
		if($projectsContainer.length){
			...
			$('#ListHeader').on('click','a',function(e){
				...
				$groupAttr = $l.attr('data-filter-group');
				if($groupAttr){$groupAttr='data-filter-'+$groupAttr;}
				...
				$projects.tsort({order:(desc?'desc':'asc'),attr:filter}).each(function(i,el){
					var $El = $(el).addClass(filter)
						, iFr = $.data(el,'h')
						, iTo = i*iLnH
						, val = $groupAttr ? $El.attr($groupAttr) : false
						;
			...
			if($itemsType == 'projects'){
				...
				$sorters.filter('.date').trigger('click');
			}
		}
